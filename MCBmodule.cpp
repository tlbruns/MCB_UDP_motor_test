...
int16_t MCBmodule::convertEffortToDAC(float effort)
{
	...
	if (effort > maxCurrent_) { effortTemp = maxCurrent_; }
	else if (effort < -maxCurrent_) { effortTemp = -maxCurrent_; }

	// compute relative effort and scale to int16 range
	return (int16_t)(32767 * (effortTemp / maxCurrent_));
}
...
void MCBmodule::setMaxCurrent(float maxCurrent)
{
	maxCurrent_ = fabsf(maxCurrent); // just want the magnitude
}

float MCBmodule::getMaxCurrent(void)
{
	return maxCurrent_;
}
